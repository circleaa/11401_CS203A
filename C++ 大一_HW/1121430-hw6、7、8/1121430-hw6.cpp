...
void compSquareRoot( long long int *hugeInt, long long int *squareRoot, int hugeIntSize, int sqrtSize )
{
   ...
   for (int i = sqrtSize - 1;i >= 0;i--)
   {
       ...
       {       
           ...
           compSquare(squareRoot, square, sqrtSize, squareSize); //square=squareRoot*squareRoot,進位
   
           if (equal(square, hugeInt, squareSize, hugeIntSize))//square=hugeInt
           {
               delete[] square;   
               return;
           }
               
           else if (less(square, hugeInt, squareSize, hugeIntSize))//hugeInt>square
               low = squareRoot[i] + 1;          
           else
               high = squareRoot[i] - 1;
       }
...
void compSquare( long long int *hugeInt, long long int *square, int hugeIntSize, int &squareSize )
{
    ...
    for (int i = 0; i < squareSize-1; i++)//進位
    {
        if (square[i] >= 10000)
        {
