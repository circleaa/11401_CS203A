...
void multiplication( int multiplicand[], int multiplier[], int product[],
                     int multiplicandDegree, int multiplierDegree, int &productDegree )
{
    if (isZero(multiplicand, multiplicandDegree) || isZero(multiplier, multiplierDegree))
    {
        productDegree = 0;
        product[0] = 0;
        return;
    ...
    for (int i = 0; i <= multiplicandDegree; i++)
        for (int j = 0; j <= multiplierDegree; j++)
            product[i + j] += multiplicand[i] * multiplier[j];//modify"+"
    while (product[productDegree] == 0)
        productDegree--;
               ...
               int dividendDegree, int divisorDegree, int &quotientDegree, int &remainderDegree )
{
   ...
   for (int n = remainderDegree - divisorDegree; n >= 0; n--)
   {
       int monomial[arraySize] = {0}, monomialDegree = n;
       int buffer[arraySize] = {0}, bufferDegree = remainderDegree;
       
       quotient[n] = remainder[remainderDegree] / divisor[divisorDegree];
       monomial[n] = quotient[n];
