...
* **無向圖 (Undirected)**：頂點的度數是與該頂點相連的邊數
* **有向圖 (Directed)**：
    * **入度 (In-degree)**：指向該頂點的邊數 (該頂點是箭頭的頭)
    * **出度 (Out-degree)**：從該頂點出發的邊數 (該頂點是箭頭的尾)
...
* **資料結構**：使用**佇列 (Queue)** (遵循 First-in, First-out (FIFO) 原則)
* **演算法核心**：
    1.  將起點頂點加入佇列和 `visited` 集合
    2.  當佇列不空時，取出頂點 $v$
