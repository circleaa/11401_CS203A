...
void MakeReservation::chooseDate( Date &date, Date currentDate, int numCustomers )
{
    ...
    AvailTableDatabase a;
    int d ,n=0;
 
    for (int i=1; i <= 30; i++)
    {
        if(availTableDatabase.availableTables(numCustomers, currentDate + i)==1)
            cout << i << ". " << currentDate + i << "   ";
        n++;
        if (n == 4)
        {
            n = 0;
...
void MakeReservation::chooseTime( int &timeCode, Date date, int numCustomers )
{
    ...
    if (availTableDatabase.availableTables(numCustomers, date, 1) == 1)
        cout << "1. 11:30"<<endl;
    if (availTableDatabase.availableTables(numCustomers, date, 2) == 1)
        cout << "2. 14:30"<<endl;
    if (availTableDatabase.availableTables(numCustomers, date, 3) == 1)
        cout << "3. 17:30"<<endl;
