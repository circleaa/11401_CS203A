...
string::string(const char* const ptr, const size_type count) // from buffer constructor
    : bx(),         // Copies the first "count" characters from the array of characters pointed by ptr.
    mySize(0),
    myRes(15)
{
    mySize = count;
    ...
    for (size_type i = 0; i < mySize; ++i) // 狡籹 right 才﹃ず甧讽玡才﹃
        myPtr()[i] = ptr[i];
    myPtr()[mySize] = '\0'; // 睰挡Ю才
}
